...
def check_accuracy(sess, correct_prediction, is_training, dataset_init_op):
    ...
    while True:
        try:
            ...
            num_correct += np.sum(np.asarray(correct_pred))
            num_samples += np.asarray(correct_pred).size
    ...
    for fold in [args.data_folds]:
        ...
        for train_filenames, train_labels, val_filenames, val_labels in data_iterator:
            ...
            with tf.Session(graph=graph) as sess:
                ...
                #     # Check accuracy on the train and val sets every epoch
                #     train_acc = check_accuracy(sess, correct_prediction, True, train_init_op)
                #     val_acc = check_accuracy(sess, correct_prediction, False, val_init_op)
                #     print('Train accuracy: %f' % train_acc)
                #     print('Val accuracy: %f\n' % val_acc)

                #     # Write summary
                #     summary = sess.run(merged, {is_training: False})
                #     writer.add_summary(summary, args.num_epochs1 + epoch + 1)
